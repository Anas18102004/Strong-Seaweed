...
def main() -> None:
    ...
    for name, fname in rasters.items():
        tagged_path = RASTER_DIR / fname
        if tagged_path.exists():
            resolved_rasters[name] = fname
            continue
        # If a tagged raster is missing, fall back in this order:
        # 1) _v1_1 aligned/versioned raster
        # 2) base aligned raster without release tag
        base_name = fname.replace(suffix, "") if suffix else fname
        fallback_candidates = [base_name]
        if suffix:
            fallback_candidates = [
                base_name.replace(".tif", "_v1_1.tif"),
                base_name.replace("_aligned.tif", "_aligned_v1_1.tif"),
                base_name,
            ]
        picked = None
        for cand in fallback_candidates:
            if (RASTER_DIR / cand).exists():
                picked = cand
                break
        if picked is not None:
            resolved_rasters[name] = picked
            continue
        missing.append(name)
    if missing:
    ...
    ref_path = RASTER_DIR / resolved_rasters["depth"]
    ...
    for name, fname in resolved_rasters.items():
        ...
        with rasterio.open(path) as src:
            ...
            if not same:
                raise ValueError(
                    ...
                    f"Expected CRS/transform/shape from {resolved_rasters['depth']}."
                )
